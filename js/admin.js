...
function renderOrdersTable() {
  ...
  body.querySelectorAll("[data-order-id]").forEach((sel) => {
    sel.addEventListener("change", () => {
      const order = allOrders.find((o) => o.id === sel.dataset.orderId);
      if (sel.value === "delivered" && order?.deliveryCode) {
        const entered = prompt(
          `Enter the delivery code the customer gave you to confirm this order as delivered:`
        );
        if (entered !== order.deliveryCode) {
          alert("That code doesn't match — order was NOT marked as delivered.");
          sel.value = order.status; // revert the dropdown
          return;
        }
      }
